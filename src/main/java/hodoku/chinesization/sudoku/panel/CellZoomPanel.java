...
import hodoku.chinesization.sudoku.entity.ColorKuImage;
...
//        System.out.println("jFOntButton: " + defaultButtonHeight + "/" + metrics.getHeight() + "/" + buttonFontSize + "/" + buttonFont + "/" + iconFont);
    ...
    private void initComponents() {
        ...
        ResourceBundle bundle = PropertyUtil.loadResourceFile("intl/CellZoomPanel"); // NOI18N

        /*
            HINT 20230916 cw 在窗体右边的解题步骤面板中，Active Cell栏中显示的“Active Cell”出现了乱码问题。通过更改字体，该问题得以解决。
            titleLabel.setFont(new Font("MS Song", Font.PLAIN, 12));
         */
        titleLabel.setFont(new Font("MS Song", Font.PLAIN, 12));
...
//        y += chooseCandidateColorLabel.getHeight();
    ...
    public void update(SudokuSet values, SudokuSet candidates, int aktColor, int index,
        ...
        if (aktColor == -1) {
            ...
            if (singleCell) {
                toggleCandidatesLabel.setText(PropertyUtil.getProperty("intl/CellZoomPanel","CellZoomPanel.toggleCandidatesLabel.text"));
                for (int i = 0; i < toggleCandidatesButtons.length; i++) {
                    toggleCandidatesButtons[i].setEnabled(true);
                }
            } else {
                toggleCandidatesLabel.setText(PropertyUtil.getProperty("intl/CellZoomPanel","CellZoomPanel.toggleCandidatesLabel.text2"));
            }
        } else {
            // coloring
